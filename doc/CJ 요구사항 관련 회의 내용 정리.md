...
~~~ 
 2016.10.19
 * 이 : 
  * 상품은 원가 정보를 가지고 있고 가격정책이 적용되면 가격정책의 가격을 우선시함(가격정책이 없어도 문제없을듯)
  * 가격정책의 값은 원가를 대체하거나 할인률을 나타냄
  * 가격정책에는 다음과 같은 정보가 있다고 생각함
   * type : 정책의 유형. 가격을 직접 변경하거나 할인률만 적용하거나...
   * value : 정책의 값. 유형에 따라 의미하는 바가 다름. 
~~~ 
